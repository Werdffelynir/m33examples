...
export class GenShapesComponent extends ReaComponent {
	...
	tunnel(renderer, scene, camera) {
		...
		const f2 = this.addFigure2()
		f2.scale.set(0.4, 0.4, 0.4)
		f2.position.set(-10, 0, -5)

		const f3 = this.addFigure3()
		f3.scale.set(0.4, 0.4, 0.4)
		f3.position.set(-5, 0, 5)

		const f4 = this.addFigure4()
		f4.scale.set(0.4, 0.4, 0.4)
		f4.position.set(0, 0, 5)


		const p1 = this.extrudePath()
		p1.scale.set(0.4, 0.4, 0.4)
		p1.position.set(5, 0, 15)
	...
	addFigure1 () {
		...
		const pathDoor = new THREE.Shape();
		pathDoor.moveTo( 4, 0 );
		pathDoor.lineTo( 4, 4 );
		pathDoor.quadraticCurveTo( 5, 5, 6, 4 );
		pathDoor.lineTo( 6, 0 );

		const figure1Geo = BuildUtil.createGeometry({
			points: [
				0, 0,
				0, 10,
				10, 10,
		...
		// figure1Geo.scale(0.5, 0.5, 0.5)
		// figure1Geo.translate(-5, 0, -5)

		const figure1 = new THREE.Mesh( figure1Geo, this.standardMat  ) ;

		this.scene.add( figure1 );
	...
	addFigure2 () {
		...
		const figure2Geo = BuildUtil.createGeometry({
			points: [
				0, 0,
				0, 10,
				10, 10,
		...
		figure2Geo.rotateX(-Math.PI / 2)

		const figure2 = new THREE.Mesh( figure2Geo, this.standardMat ) ;
		this.scene.add( figure2 );
	...
	addFigure3 () {
		...
		// hole
		const pathPip = new THREE.Shape();
		pathPip.absellipse( 7, 7,  2, 3)
		// pathPip.moveTo( 10, 3 );
		// pathPip.lineTo( 6, 3 );
		// pathPip.lineTo( 6, 6 );
		// pathPip.lineTo( 10, 6 );
		...
		// extrudePath
		// LineCurve3 CatmullRomCurve3 QuadraticBezierCurve3 CubicBezierCurve3
		...
		const patWallOffset = new THREE.Shape();
		patWallOffset.setFromPoints([
		...
		const pathDoorOutter = new THREE.Shape();
		pathDoorOutter.setFromPoints([
		...
		const figureGeo = BuildUtil.createGeometry({
			points: basis,
			holes: [
				patWallOffset,
				// pathDoorOutter,
		...
		figureGeo.scale(0.5, 0.5, 0.5)
		// figureGeo.translate(-5, 0, -5)

		const figure = new THREE.Mesh( figureGeo, this.standardMat  ) ;
	...
	addFigure4 () {
		...
		const pathWall = new THREE.Shape();
		pathWall.moveTo( 0, 0 );
		pathWall.lineTo( 0, 20);
		pathWall.lineTo( 20, 20);
		pathWall.lineTo( 20, 0);
		pathWall.lineTo( 0, 0);

		const pathInside = new THREE.Shape();
		const waypoints = [1,0,1,4,5,4,5,6,3,6,3,9,2,9,2,15,8,15,8,9,5,9,5,8,11,8,11,9,9,9,9,13,16,13,16,9,14,9,14,8,17,8,17,11,19,11,19,7,16,7,16,5,18,5,18,1,11,1,11,3,3,3,3,0,1,0];
		
		pathInside.setFromPoints( BuildUtil.createPointPath(waypoints, 2) )
	...
	extrudePath () {
		...
		const enterPath = new THREE.Path()
		enterPath.moveTo(-0.95, -0.95)
		enterPath.lineTo( 0.95, -0.95)
		enterPath.lineTo( 0.95,  0.95)
		enterPath.lineTo(-0.95,  0.95)
		enterPath.closePath()

		shape.holes.push(enterPath)
